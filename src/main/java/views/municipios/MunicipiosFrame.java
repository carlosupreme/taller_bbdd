...
public final class MunicipiosFrame extends javax.swing.JFrame {

    private final MunicipioRepository municipioRepository;
    private final DefaultTableModel model;
    private final Entidad entidad;

    public MunicipiosFrame(Entidad entidad) {
        initComponents();
        jLabel1.setText("Municipios de " + entidad.getNombre());
        ...
        TableActionEvent actionEvent = new TableActionEvent() {
            ...
            public void onEdit(int row) {
                ...
                new EditarMunicipioModal(MunicipiosFrame.this, municipioRepository,entidad.getId(), id).setVisible(true);
            }
            ...
            public void onDelete(int row) {
                ...
                long id = (long) model.getValueAt(row, 0);
                try {
                    Municipio municipioToDelete = municipioRepository.findById(entidad.getId(), id);
                    boolean confirmed = MessageHandler.showConfirmMessage("¿Estás seguro de que desea eliminar el municipio '" + municipioToDelete.getNombre() + "' ?", "Eliminar permanentemente");

                    if (confirmed) {
                        municipioRepository.delete(entidad.getId(), id);
                        loadEntries();
                        MessageHandler.showSuccessMessage("Municipio eliminado", null);
                    }
            ...
            public void onShow(int row) {
                long id = (long) model.getValueAt(row, 0);
                try {
                    new VerMunicipioModal(MunicipiosFrame.this, municipioRepository.findById(entidad.getId(), id)).setVisible(true);
                } catch (SQLException | ValidationModelException ex) {
                    MessageHandler.showErrorMessage(ex.getMessage());
                }
            }
    ...
    public void loadEntries() {
        model.setRowCount(0);
        try {
            municipioRepository.findAll(entidad.getId()).forEach(municipio -> {
                Object[] row = new Object[3];
                row[0] = municipio.getId();
                row[1] = municipio.getNombre();
                row[2] = "";
                model.addRow(row);
            });
        } catch (SQLException | ValidationModelException ex) {
    ...
    private void crearBtnActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_crearBtnActionPerformed
        new CrearMunicipioModal(this, municipioRepository, entidad).setVisible(true);
